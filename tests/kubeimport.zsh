...
#!/bin/sh
printf '%s %s\n' "$1" "$2" >> "$HOME/.kctx-status-calls"
EOF
chmod +x "$test_home/kctx-status"
export KCTX_STATUS_SCRIPT="$test_home/kctx-status"
kctx selected-context >/dev/null
if [[ "$(<"$test_home/.kctx-status-calls")" != 'connect selected-context' ]] ||
   [[ "$(<"$test_home/.kubectl-used-context")" != selected-context ]] ||
   [[ "$(sed -n 's/^current-context: *//p' "$test_home/.kube/agent-context")" != selected-context ]]; then
  print -u2 "expected kctx to switch immediately and queue a background check"
  exit 1
fi
